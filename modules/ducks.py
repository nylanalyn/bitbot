...
class Module(ModuleManager.BaseModule):
    ...
    @utils.hook("received.command.rel", alias_of="release")
    ...
    @utils.hook("received.command.free")
    @utils.kwarg("help", "Release a befriended duck")
    @utils.spec("!-channelonly")
    def free(self, event):
        user_id = event["user"].get_id()
        channel = event["target"]
        befriended_ducks = channel.get_user_setting(user_id, "ducks-befriended")
        if befriended_ducks and befriended_ducks > 0:
            befriended_ducks -= 1
            channel.set_user_setting(user_id, "ducks-befriended", befriended_ducks)
            event["stdout"].write("%s freed a duck!" % event["user"].nickname)
            self._trigger_duck(channel)
        else:
            event["stderr"].write("You don't have any friendly ducks!")
